...
function mergeObjects(objects) {
  ...
  Object.keys(obj2).forEach((key) => {
    res[key] = Object.prototype.hasOwnProperty.call(obj1, key)
      ? res[key] + obj2[key]
      : (res[key] = obj2[key]);
  });
  return res;
...
function makeWord(obj) {
  ...
  const result = Array([].concat(...Object.values(obj)).length);

  keys.forEach((key) => {
    obj[key].forEach((item) => {
      result.splice(item, 1, key);
    });
  });
  return result.join('');
}
